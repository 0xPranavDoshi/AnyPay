...
function getBillSplittingPrompt(state: ConversationState, data: BillSplittingData, userPrompt: string): string {
  const systemPrompts = {
...
function determineConversationState(history: any[], billData: BillSplittingData): ConversationState {
  if (history.length === 0) return 'INITIAL';
  
  if (!billData.totalAmount || !billData.peopleCount) return 'WAITING_FOR_PEOPLE_COUNT';
  if (!billData.users || billData.users.length !== billData.peopleCount) return 'WAITING_FOR_USERNAMES';
  ...
  for (const message of history) {
    if (message.role === 'model' && message.parts[0]?.text) {
      ...
      if (!data.totalAmount) {
        ...
        const totalLabelRegex = /(total|amount\s*due|grand\s*total)[^\$]*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i;
        ...
        if (totalLabelMatch) {
          ...
          const allAmounts = (text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)/g) || [])
    ...
    if (message.role === 'user' && message.parts[0]?.text) {
      ...
      if (text.includes('paid') && data.users && !data.payer) {
        const paidUser = data.users.find(user => text.includes(user.username.toLowerCase()));
        if (paidUser) data.payer = paidUser;
      }
      
      // Extract split method
      if ((text.includes('equal') || text.includes('evenly')) && !data.splitMethod) {
...
function validateSettlement(settlement: Payment): boolean {
  ...
  const sumRecipients = settlement.recipients.reduce((acc, r) => acc + r.amount, 0);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const conversationState = determineConversationState(session.history, billData);
    ...
    if (isFirstPrompt) {
      ...
      if (!image) {
        return NextResponse.json({ error: 'Receipt image is required for the first prompt' }, { status: 400 });
      }
      ...
      try {
        ...
        return NextResponse.json({ error: 'Agent is unreachable at the moment' }, { status: 502 });
      }
    } else {
    ...
    const systemPrompt = getBillSplittingPrompt(conversationState, billData, prompt);
    ...
    try {
      if (typeof imageDataForGemini !== 'undefined') {
        geminiResponse = await callGemini(enhancedPrompt, imageDataForGemini, session.history);
      } else {
        geminiResponse = await callGemini(enhancedPrompt, undefined, session.history);
      }
      ...
      return NextResponse.json({ error: 'Agent is unreachable at the moment' }, { status: 503 });
    }

    // 5. Check if user is confirming settlement
    let paymentId: string | undefined;
    const updatedBillData = extractBillSplittingData([...session.history, 
      { role: 'user', parts: [{ text: prompt }], timestamp: new Date() },
      { role: 'model', parts: [{ text: geminiResponse.text }], timestamp: new Date() }
    ]);
    
    if (prompt.toLowerCase().includes('confirm') && conversationState === 'WAITING_FOR_CONFIRMATION') {
      const settlement = generateSettlement(updatedBillData);
      if (!settlement || !validateSettlement(settlement)) {
        return NextResponse.json({ error: 'Agent is unreachable at the moment' }, { status: 422 });
      }
      try {
        ...
        return NextResponse.json({ error: 'Agent is unreachable at the moment' }, { status: 500 });
      }
    }
