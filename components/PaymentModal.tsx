...
export default function PaymentModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const priceDiff =
        balanceResult.requiredBalance - balanceResult.currentBalance;
  ...
  const availableTokens = sourceChain
    ? CHAINS[sourceChain as keyof typeof CHAINS].tokens
    : [];
    ...
    <>
          ...
          <select
            ...
            onChange={(e) =>
              setTokenType(
                e.target.value === ""
                  ? ""
                  : (parseInt(e.target.value) as TokenType)
              )
            }
        ...
        {sourceChain && tokenType !== "" && (
          <div className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-lg p-3">
            ...
            <div className="text-sm text-[var(--color-text-primary)]">
              • Send {TOKEN_NAMES[tokenType as TokenType]} from{" "}
            ...
            sourceChain === destinationChain ? (
              ...
              <>
                <div className="text-sm text-[var(--color-text-primary)]">
                  • Recipient receives USDC on{" "}
                  {CHAINS[destinationChain as keyof typeof CHAINS]?.name ||
                    "Base Sepolia"}
                </div>
                <div className="text-sm text-[var(--color-text-primary)]">
                ...
                <div className="text-xs text-[var(--color-text-muted)] mt-1">
                  {TOKEN_NAMES[tokenType as TokenType]} on{" "}
                  {CHAINS[sourceChain as keyof typeof CHAINS]?.name} → USDC on{" "}
                  {CHAINS[destinationChain as keyof typeof CHAINS]?.name}
                </div>
              </>
            )}
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          disabled={
            ((!sourceChain || tokenType === "") && walletConnected) ||
            isProcessing
          }
        ...
        >
          {isProcessing
            ...
            ? "Connect MetaMask"
            : currentChainId !==
                `0x${parseInt(sourceChain || "0").toString(16)}` && sourceChain
  ...
  const renderInsufficientBalanceScreen = () => {
    ...
    console.log(
      "Rendering insufficient balance screen with balanceInfo:",
      balanceInfo
    );
          ...
          <div className="bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-xl p-4">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="font-semibold text-[var(--color-text-primary)]">
                {balanceInfo?.requiredBalance || "N/A"}{" "}
                {balanceInfo?.tokenSymbol || "Token"}
              </span>
            </div>
            <div className="flex justify-between items-center mb-3">
              <span className="text-sm text-[var(--color-text-muted)]">
            ...
            <div className="border-t border-[var(--color-border)] pt-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-[var(--color-primary)]">
                  {(
                    parseFloat(balanceInfo?.requiredBalance || "0") -
                    parseFloat(balanceInfo?.currentBalance || "0")
                  ).toFixed(6)}{" "}
                  {balanceInfo?.tokenSymbol || "Token"}
                </span>
              </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-6 max-w-md w-full mx-4">
        {currentScreen === "payment"
          ? renderPaymentScreen()
          : renderInsufficientBalanceScreen()}
      </div>
    </div>
  );
}
